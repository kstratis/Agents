//A studious InfoSpeak student - ABS assignments 2011/12
/* Initial beliefs */
...
+!attendlectures(Me) : week(WeekNow) & day(Day) & time(TimeNow) & TimeNow > 9 & TimeNow < 18  <- !pickassignment(Me);!doassignment(Me); !checkevents(Me).
+!attendlectures(Me) : week(WeekNow) & day(Day) & time(TimeNow) & TimeNow >= 18 <- !gotolibrary(Me); add_goal("Revising in library.");!checkevents(Me).
...
+!setassignment(Me,X): X == [2**31-1,8,25] <- !gotolibrary(Me); add_goal("No assignments left!Going to revise in library!");!checkevents(Me).
...
+!doassignment(Me): time(TimeNow) & pickedassignment(Course,Num,Sweek,Sday,Stime,Eweek,Eday,Etime,H) <- !gotolab(Me);.concat("Working on ",Course, " assignment,",Num," and have ",H," hours left!",Y);add_goal(Y);work_one_hour(TimeNow,H);!update(Me).
...
+!gotolibrary(Me): pos(lib1,X,Y) & pos(Me,Xm,Ym) & not(X == Xm & Y == Ym)<- add_goal("Going to the library."); !goto(Me,X,Y).
+!gotolibrary(Me): pos(lib1,X,Y) & pos(Me,Xm,Ym) & X == Xm & Y = Ym.

+!gohome(Me): home(Place) & pos(Place,X,Y) & pos(Me,Xm,Ym) & not(X == Xm & Y == Ym) <- add_goal("Going home.");!goto(Me,X,Y).
+!gohome(Me): home(Place) & pos(Place,X,Y) & pos(Me,Xm,Ym) & X == Xm & Y == Ym.
